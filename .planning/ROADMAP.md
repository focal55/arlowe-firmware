...
**Success Criteria** (what must be TRUE):
  ...
  6. On a freshly flashed image, all six units reach `active` — hardware checkpoint, deferrable per Phase 1/3/4/5 precedent, but recorded as unproven until it runs.
     **STATUS: MET 2026-09-25**, from a clean flash of an image built from `6a4c538`: all six `active`, no hand changes. Evidence: `docs/operations/phase-7.1-substrate.md` §SC6 re-run from a clean image.
...
- [x] 07.1-06-PLAN.md — SC6: substrate runbook (`docs/operations/phase-7.1-substrate.md`) + ROADMAP/REQUIREMENTS traceability delivered; the SC6 hardware checkpoint was not run in this plan; **it has since been run and met, 2026-09-25** (Wave 4; non-autonomous, deferrable per Phase 1/3/4/5 precedent, depends on 07.1-01..05)
...
**Phase 7.1 closed the substrate, which is what this phase's SC2 was blocked on.** SC2 requires the pairing daemon to "start the runtime services". Before 7.1 that criterion was unreachable: `/opt/arlowe/venvs` was empty, `dashboard/server.js` did not exist, and no build step produced either — so a failing SC2 would have been ambiguous between "pairing is broken" and "the services were never startable". That ambiguity is the stated reason 7.1 was inserted rather than folded into this phase. **A Phase 8 SC2 failure now means pairing is broken.** 7.1's SC6 is met on hardware from a clean image (2026-09-25), so that holds on a device, not only in containers.
